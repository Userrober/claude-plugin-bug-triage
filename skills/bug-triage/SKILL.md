...
- ❌ **Don't fabricate a causal chain between real code anchors and the bug.** Finding `isCurrentThemeV2()` in the repo is real. Claiming "the mobile theme bug is caused by `isCurrentThemeV2()` not being gated for mobile audience" — when you have NO evidence connecting that gate to this specific bug — is a hallucinated diagnosis dressed in real file paths. Every individual anchor is verifiable, but the *story linking them to the symptom* is fiction. Inferences are allowed if you mark them as inferences ("appears to", "likely", "inferred from <evidence>") — fabricated certainty is not. The enrichment's job is to surface evidence and reason from it, not invent narratives.
- ❌ **Don't bail out to NO-REPRO without doing the context dig first.** Most bugs without a stack trace are still INVESTIGABLE — they have linked IcMs, related work items, feature tags, quoted strings, identifiers that point at code, OR people-dimension signal (assignee's recent PRs reveal ownership; creator's other bugs reveal a cluster; a PR already references the bug). Falling back to "Status: NO REPRO YET" should be rare (<10% of bugs). If you're reaching for the NO-REPRO template more than 1 in 10 enrichments, you're under-investigating: read the IcM, read related/parent bugs, search wiki for the feature, code-search the most specific identifier, check recent commits, AND check assignee/creator/reporter's recent PRs and other bugs *before* deciding there's no signal. See `enrichment-agent-prompt.md` Honesty Gate for the mandatory 8-step dig.
