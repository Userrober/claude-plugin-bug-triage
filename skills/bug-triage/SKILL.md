...
- ❌ **Don't bail out to NO-REPRO without doing the context dig first.** Most bugs without a stack trace are still INVESTIGABLE — they have linked IcMs, related work items, feature tags, quoted strings, or identifiers that point at code. Falling back to "Status: NO REPRO YET" should be rare (<10% of bugs). If you're reaching for the NO-REPRO template more than 1 in 10 enrichments, you're under-investigating: read the IcM, read related/parent bugs, search wiki for the feature, code-search the most specific identifier, and check recent commits *before* deciding there's no signal. See `enrichment-agent-prompt.md` Honesty Gate for the mandatory dig steps.
